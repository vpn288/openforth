...
_push:
        ...
        mov rbx,[r10+65536+32] ; stack pointer
        add rbx , cell_size
        and rbx , [data_stack_mask]
        add r10,rbx
        mov [r10] , rax
        mov [r11+65536+32],rbx
        ret
        align  cell_size
thread_base    dq       0
...
_pop:
  ...
  ;      mov     [stack_base],rax
        ...
        mov r10,rax ;[stack_base]
        mov rbx,[r10+65536+32]

        mov rax , [rbx+r10]
        sub rbx , cell_size
        and rbx , [data_stack_mask]
        mov [r10+65536+32],rbx
        ret
;------------------------------
        align cell_size
nfa_5:
...
_word4:
; string to validate
;çäåñü ñëîâî íà ãðàíèöå     ñäâèíåì ôàéë ïîéíòåð íà íà÷àëî ñëîâà.
   ;    int3
        sub    rsp,30h
        mov     rcx,[handle]
...
;        mov     rdx, [buffer]
        ...
        add     rsp,30h
         mov     rax,[handle]
        call    _push
        call    _rdfile   ;      à òåïåðü íàäî ïåðåìåñòèòü èíäåêñû
        ;ïåðåìåííóþ IN> ñíîâà íà 0, rdi íàçàä, rsi íà íà÷àëî áóôåðà.
        mov         rdi,r13
        mov         rsi,r12
        xor         rdx,rdx
        mov         [_in_value],rdx
...
file_pos             dq 0
                     dd 0
                      ...
                      db 0
fileptr              dq filename ;¢¬¥áâ® ­®¬¥à  ¡«®ª  - ãª § â¥«ì ­  ¨¬ï ä ©« 
